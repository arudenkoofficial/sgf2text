...
- [x] 6.5 Re-send the `?lang=ru` link on the live host to the players who use the tool. Task 7.7 of `localise-page-metadata` sent a link before the deploy; if that link named the github.io host it lands on `http`, so the cookie was never stored and the choice was forgotten. This is the visit where she finds out. Sent, on `https://sgf.rudenko.live/?lang=ru`.

  This closes task 7.7 of `localise-page-metadata`, which was archived open. It was the one task meant to precede that deploy, its shape changed once the custom domain was found, and it travelled here as 6.5. Both changes are now delivered rather than one of them carrying the other's unfinished ending.

  What the link does that the previous one could not: it lands on `https` directly, with no redirect hop, so the browser keeps the `Secure` language cookie. A reader who opens it once gets Russian, and gets Russian again on a later visit to the bare address — which is what the whole resolution chain was built for and what plain `http` had been silently undoing.
