...
- [ ] 7.4 View source on the deployed page — not the inspector, which shows the DOM after script — and confirm the English metadata is what is actually served.
- [ ] 7.5 Listen to the page once with a screen reader, opened through `?lang=ru`, to hear which title is announced on load, and record in the change whether the deferred module swaps it late enough to matter.
